...
const Projects = () => {
    ...
    const nextSlide = () => {
        const maxIndex = Math.max(0, filteredProjects.length - 3); // Assuming 3 cards are visible
                    ...
                    <div className="relative">
                        <div ref={scrollContainerRef} className="overflow-x-auto scroll-smooth snap-x snap-mandatory hide-scrollbar">
                            <div className="flex gap-6 pb-4">
                                {filteredProjects.map((project, index) => (
                                    <div key={project.id} className="w-full wd:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] shrink-0 snap-start ">
                        ...
                        {filteredProjects.length > 3 && (
                            <>
                                <button
                                    onClick={prevSlide}
                                    className="flex absolute left-0 top-1/2 -translate-y-1/2 lg:translate-x-4 items-center justify-center w-10 h-10 lg:w-12 lg:h-12 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full hover:bg-white/20 transition-all duration-300  disabled:opacity-50 disabled:cursor-not-allowed z-10  "
                                ...
                                <button
                                    onClick={nextSlide}
                                    className="flex absolute right-0 top-1/2 -translate-y-1/2 lg:translate-x-4 items-center justify-center w-10 h-10 lg:w-12 lg:h-12 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full hover:bg-white/20 transition-all duration-300  disabled:opacity-50 disabled:cursor-not-allowed z-10"
                                    disabled={
                                        currentIndex >=
                                        filteredProjects.length - 3
                                    }
                            ...
                            <div className="flex items-center justify-center gap-2 mt-8">
                                {Array.from({
                                    ...
                                    <button
                                        ...
                                        className={`transition-all duration-300 rounded-all ${
                                            index === currentIndex
                                                ? "bg-primary w-6 h-2"
